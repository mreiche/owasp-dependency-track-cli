...
def retry(_callable: Callable, seconds: float, wait_time: float = 3):
...
def validate_project_uuid(client: Client, args):
    def _find_project():
        _project = api.find_project_by_name(
            client=client,
            name=args.project_name,
            version=args.project_version,
            latest=args.latest,
        )
        validate(_project is not None, f"Project not found: {args.project_name}:{args.project_version} {' [latest]' if args.latest else ''}")
        log.LOGGER.info(f"Found project {_project.name}:{_project.version} (UUID: {_project.uuid}){' [latest]' if _project.is_latest else ''}")
        return _project
